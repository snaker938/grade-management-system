...
/**
 * Controller for managing modules, including registering/removing students and
...
public class ModuleController {
   ...
   */
  public ModuleController(ModuleRepository moduleRepo, StudentRepository studentRepo,
      RegistrationRepository regRepo, GradeRepository gradeRepo) {
    this.moduleRepo = moduleRepo;
    this.studentRepo = studentRepo;
    this.regRepo = regRepo;
    this.gradeRepo = gradeRepo;

    // Use this.gradeRepo in some way to avoid a warning
  ...
  public ResponseEntity<?> registerStudent(@PathVariable String code, @RequestBody Map<String, String> body) {
    ...
    Optional<Module> modOpt = moduleRepo.findById(code);
    Optional<Student> studOpt = studentRepo.findById(studentId);
    ...
    if (enrolledCount >= m.getMaxSeats()) {
      return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Module capacity reached");
    }

    boolean alreadyRegistered = m.getRegistrations().stream()
        .anyMatch(r -> r.getStudent().getId().equals(s.getId()));
    if (alreadyRegistered) {
      return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Student already registered in this module");
    }
  ...
  public ResponseEntity<?> removeStudent(@PathVariable String code, @PathVariable Long id) {
    Optional<Module> modOpt = moduleRepo.findById(code);
    Optional<Student> studOpt = studentRepo.findById(id);
    ...
    if (!regOpt.isPresent()) {
      return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Student not registered in this module");
    }
  ...
  /**
   * Retrieves a list of enrolled students for a specified module along with their
   * grades (if available).
   *
   * Returns a JSON object containing "enrolledStudents" as a list of student
   * details, including:
   ...
   *         grades,
   *         NOT_FOUND if the module does not exist
   */
  @GetMapping("/{code}/registrations")
  public ResponseEntity<?> getModuleRegistrations(@PathVariable String code) {
    Optional<Module> modOpt = moduleRepo.findById(code);
    if (!modOpt.isPresent()) {
      return ResponseEntity.notFound().build();
    }
    ...
    Module m = modOpt.get();
    List<Map<String, Object>> enrolledStudents = m.getRegistrations().stream().map(reg -> {
      Student st = reg.getStudent();

      Optional<Grade> gOpt = m.getGrades().stream()
          .filter(g -> g.getStudent().getId().equals(st.getId()))
          .findFirst();
   ...
   */
  @PutMapping("/{code}")
  public ResponseEntity<Module> updateModule(@PathVariable String code, @RequestBody Map<String, String> body) {
    Optional<Module> modOpt = moduleRepo.findById(code);
    if (!modOpt.isPresent()) {
      return ResponseEntity.notFound().build();
    }
